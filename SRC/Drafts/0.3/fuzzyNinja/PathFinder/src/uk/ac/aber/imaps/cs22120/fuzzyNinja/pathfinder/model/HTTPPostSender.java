/* 
 * Author: Joshua C Moss
 * Date Created:
 * Date Last Modified:
 * Co Authors:
 * 
 * 
 * */

/**package uk.ac.aber.imaps.cs22120.fuzzyNinja.pathfinder.model;

public class HTTPPostSender implements Runnable {

	
	@Override
	public void run() {
		// TODO Auto-generated method stub
		// gingerbreadman
	}
	
	private void trySend(){
		
	}
	
	private boolean hasNetwork(){
		//use this to detect whether the phone has network access. Or hack and try sending, catching any error and dealing.
		
		return true; //or false
	}
	
	public HTTPPostSender()
	{
		
		
		
	}
	public void buildString(Route postRoute)
	{
		
		
		
	}
	
	public void addString(RouteLocation concat)
	{

	}
	
	public void getString()
	{
		
	}


}
**/
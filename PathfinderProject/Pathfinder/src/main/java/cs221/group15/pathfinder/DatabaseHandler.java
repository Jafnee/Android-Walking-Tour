...
class DatabaseHandler extends SQLiteOpenHelper{
    ...
    */
    ...
    public Waypoint[] getAllWaypoints(int walkID) {
        SQLiteDatabase db = this.getWritableDatabase();

        String query = "SELECT * FROM " + TABLE_WAYPOINT + " WHERE " + KEY_WALK_ID + "=" + String.valueOf(walkID) + " LIMIT 1";
        Cursor res = db.rawQuery(query, null);
        Waypoint[] data = null;
        int i = 0;
        if(res.getCount() > 0) {
            ...
            while(res.moveToNext()) {
                ...
                String title = res.getString(res.getColumnIndex(KEY_NAME));
                String desc = res.getString(res.getColumnIndex(KEY_DESCRIPTION));
